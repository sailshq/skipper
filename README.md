...
acceptFile   | ((function))                     | Optional. This function will be called repeatedly- each time a new incoming file upload arrives on the upstream. Function signature: `acceptFile: function (fileMetadata,cb){}`, where `fileMetadata` is a simple object with three properties: `size` a numeric size (in bytes),`type` a string type (MIME type), `filename` string filename. To accept an incoming file, call `cb(null, true)`. To ignore a file, call `cb(null, false)`. And to terminate, giving up on the entire file upload request altogether, call `cb(new Error('some helpful error'))`.The error will be passed to the callback to `.upload()`
