...
module.exports = function write (fileStream) {
	...
	fileStream.on('data', function receiveBinaryData(newBytes) {
		...
		} else logmsg += 'Field stream :: "' + self.fieldName + '" ';
		...
		var fileRecord = self.files[fileStream._id];
		...
		self._bytesWritten += newBytes.length;

		// Whenever a new update comes in, check that upload limit
		// constraints are not violated- if they are, break the stream
		var valid = true;
		valid = valid && self.enforceMaxBytes(self._bytesWritten, this);
		valid = valid && self.enforceMaxBytesPerFile(fileRecord.size, this);
		...
		self.onFileData(newBytes, fileStream);
	});
