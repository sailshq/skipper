...
Upstream.prototype._read = function onNewDataRequested ( numBytesRequested ) {
	//
	// Don't really need to do anything in here for now--
...
Upstream.prototype.upload = function ( receiver__, cb ) {
	...
	receiver__.once('finish', function allFilesUploaded (files) {
		log('Receiving writestream finished.'.grey);
		cb(null, files);
	});
	...
	receiver__.once('error', function unableToUpload (err, files) {
		log(('Receiving writestream encountered error :'+util.inspect(err)).red);
		cb(err);
	});
...
Upstream.prototype.writeFile = function ( filestream ) {
	...
	filestream.once('error', function (err) {
		...
		// connectedTo anything), then we shouldn't emit an error on it
		// (since it would cause the server to crash).
		// We'll sort of just... do nothing. (the error event emitted on the actual Upstream
		// itself will, worst case, log a warning message.)
		if (!self.connectedTo || !self.connectedTo.length) {
		  return;
		}
		...
		// receivers, we should let each of the receivers' file write streams
		// handle the file "WRITE" error however they see fit:
		// if (!self.connectedTo || !self.connectedTo.length) {
			// _.each(this.connectedTo, function eachReceiver(outs) {
			//   _.each(outs._files, function (fouts) {
			//     fouts.emit('error', err);
			//   });
			// });
			// return;
		// }
...
Upstream.prototype.fatalIncomingError = function (err) {
	...
	log(('Fatal incoming error in MPU form ::'+util.inspect(err)+' (user may have cancelled the request..?)  Aborting/cancelling all future, current, and previously uploaded files from this request!').red);
	...
	_(this._files).each(function (file) {
		...
		log(('Upstream: "READ" error on incoming file `'+file.filename+'` ::'+err).red);
	});
