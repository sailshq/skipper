...
Upstream.prototype.upload = function ( receiver__, cb ) {
  ...
  if (!isValid) {
    ...
    if (typeof receiver__ === 'object') {
      ...
      filename_param = (receiverOpts.filename !== undefined) ? receiverOpts.filename : "";
    }
      ...
      if (receiver__.match(/^\//)) {
        ...
        filename_param = receiverOpts.filename;
      }
      else {
  ...
  receiver__.saveAs(filename_param,function generateNameFinish(err) {
      if(err){
          return cb(err);
      }
...
Upstream.prototype.serializeFiles = function ( ) {
  ...
  return _.reduce(self._files, function (memo, file) {
    ...
    var fileid = path.basename(file.stream._readableState.pipes.path);
    if(fileid !== file.stream.filename){
        fileinfo.id = fileid;
    }
