...
var Readable = require('stream').Readable
	, Writable = require('stream').Writable // (for the leaky pipe)
	, _ = require('lodash')
	, log = require('./logger')
	, util = require('util');
...
Upstream.prototype.pipe = function () {
	this._connected = true;
	Readable.prototype.pipe.apply(this, Array.prototype.slice.call(arguments));
};


/**
 * Override read so we know when this starts getting consumed.
 */

// Upstream.prototype.read = function () {
// 	this._connected = true;
// 	Readable.prototype.read.apply(this, Array.prototype.slice.call(arguments));
// };
...
Upstream.prototype.writeFile = function ( filestream ) {
	...
	filestream.once('error', function (err) {
		...
		// happens for everything to work.  Otherwise, responses never get sent.
		var leaky = new Writable();
		leaky._write = function (chunk,encoding, cb) { cb(); };
		filestream.unpipe();
		filestream.pipe( leaky );
		console.log('Piping incoming file `'+filestream.filename+'` to the memory hole..');
	});
