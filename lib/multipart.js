...
module.exports = function toParseMultipartHTTPRequest (options) {
	...
	return function _parseMultipartHTTPRequest (req, res, next) {
		...
		form.on('field', function (field, value) {
			...
			console.log('got field ', '`'+field+'`','::',req.body[field]);
		});

		// Wait to pass control to the app until one of the following is true:
		// + request body has been completely parsed
		// + at least one file is received
		// + maxTimeToWaitBeforePassingControl ms have elapsed w/o receiving even one file
		var maxTimeToWaitBeforePassingControl = 50;
		console.log('---');
		setTimeout(function () {
			console.log('Passing control to app...');
			next();
		}, maxTimeToWaitBeforePassingControl);
	};
};
