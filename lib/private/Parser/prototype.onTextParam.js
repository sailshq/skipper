 ...
 * >
 * > This problem is worked around in Sails/parasails/Cloud SDK by automatically
 * > appending a special header to the request called "X-JSON-MPU-Params" which
 * > identifies the names of text parameters that are sent as stringified JSON
 * > and thus should be parsed as such on the server.
 * > - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
...
module.exports = function onTextParam(part) {
  ...
  // > (see note at top of this file about the new approach for encoding nested
  ...
  part.on('readable', function onBytesAvailable() {
    ...
    // TODO: Pull in implementation with "X-JSON-MPU-Params" from machine-as-action to instead live right here
