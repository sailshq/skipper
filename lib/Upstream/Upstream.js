...
Upstream.prototype.upload = function(receiver__, cb) {
  ...
  if (!isValid) {
    ...
    if (typeof receiver__ === 'object') {
      ...
      //       } else {
      // =======
      desiredFilename = (receiverOpts.filename !== undefined) ? receiverOpts.filename : "";
    } else if (typeof receiver__ === 'string') {
      if (receiver__.match(/^\//)) {
        receiverOpts.filename = path.basename(receiver__);
        receiverOpts.dirname = path.dirname(receiver__);
        desiredFilename = receiverOpts.filename;
      } else {
        // >>>>>>> 167cd23c3a03ddab742ab31c4ad9d47b54c01cb1:lib/Upstream.js
  ...
  // Call saveAs-Function at the recierver
  receiver__.saveAs(desiredFilename, function generateNameFinish(err) {
    if (err) {
      return cb(err);
    }
...
Upstream.prototype.serializeFiles = function() {
  ...
  return _.reduce(self._files, function(memo, file) {
    ...
    // If uploaded file have got another filename (e.g. local filesystem overwrite check)
    try {
      var fileid = file.stream.filename_write;
      if (fileid !== file.stream.filename && fileid !== undefined) {
        fileinfo.id = fileid;
      }
    } catch (e) {}
